...
(async () => {
  ...
  await page.reload(); await ready(); assert.equal(await page.locator('#sel').inputValue(),'cv_1000');
  ...
  console.log('PASS: 8 scenes / 4 models; no eager video or image preload; cache; confidence; playback; reload; failure/retry; race; mobile; no JS errors');
  await browser.close();
